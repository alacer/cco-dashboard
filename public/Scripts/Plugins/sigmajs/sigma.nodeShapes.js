...
/*
 * this function expects the following hash on node.shape:
 * {
 *   name: equilat-poly
...
sigma.tools.drawEquilatPoly = function(ctx,color,x,y,size,node) {
  ctx.fillStyle = color;
  ctx.beginPath();
  var pcount = node.shape.numPoints || 5;
  var rotate = (node.shape.rotate || 0)*Math.PI/180;
  var radius = size;
...
sigma.tools.drawStar = function(ctx,color,x,y,size,node) {
  ...
  ctx.beginPath();
  var pcount = node.shape.numPoints || 5;
  var inRatio = node.shape.innerRatio || 0.5;
  var outR = size;
...
sigma.nodeShapes.NodeShapes = function(siginst,plotter) {
  ...
  this.init = function() {
    // maintain original node-drawing functions, as shape drawing for 'circle'
    ...
    // override node-drawing, with wrappers that switch on node.shape,
   ...
   * @param {string} name of shape, to be used on node.shape
   ...
   * this function expects the following hash on node.image:
   * {
   *   url: url to image
  ...
  function drawImageOnNode(ctx, color, x, y, size, node) {
    if(self.p.drawImages && node.image && node.image.url) {
      var url = node.image.url;
      var ih = node.image.h || 10; // 10 is arbitrary, anyway only the ratio counts
      var iw = node.image.w || 10;
      var scale = node.image.scale || 0.85;
      ...
      if(!image) {
        ...
        image.onload = function(){
          ...
          //self.sigInst.draw(); // COSTLY
          imgCacheLoaded[url] = 1;
        };
  ...
  function drawNodeShapeWImage(ctx, color, x, y, size, node) {
    ...
    if(node && node.shape && node.shape.name) {
      functions = shapeFunctions[node.shape.name];
    }
